...
def disableAlerting(
    ...
    """
    url = f"https://backstory.googleapis.com/v2/detect/rules/{rule_id}:disableAlerting"
...
def disable_alerting_tool(environment: str = "default", rule_id: str = "") -> dict:
    """Disables alerting for a specified Chronicle detection rule.

    Args:
        environment (str, optional): The environment from which to fetch credentials. Defaults to "default".
        rule_id (str): The ID of the rule for which to disable alerting.
    ...
    try:
        ...
        result = disableAlerting(credentials, rule_id)

        return {"status": "success", "result": result, "message": f"Alerting disabled for rule ID: {rule_id}"}
    except requests.exceptions.HTTPError as e:
        return {"status": "error", "error_message": f"HTTP error disabling alerting: {e}"}
