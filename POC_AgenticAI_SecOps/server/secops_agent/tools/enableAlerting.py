...
def enableAlerting(
    ...
    """
    url = f"https://backstory.googleapis.com/v2/detect/rules/{rule_id}:enableAlerting"
...
def enable_alerting_tool(environment: str = "default", rule_id: str = "") -> dict:
    """Enables alerting for a specified Chronicle detection rule.

    Args:
        environment (str, optional): The environment from which to fetch credentials. Defaults to "default".
        rule_id (str): The ID of the rule for which to enable alerting.
    ...
    try:
        ...
        result = enableAlerting(credentials, rule_id)

        return {"status": "success", "result": result, "message": f"Alerting enabled for rule ID: {rule_id}"}
    except requests.exceptions.HTTPError as e:
        return {"status": "error", "error_message": f"HTTP error enabling alerting: {e}"}
