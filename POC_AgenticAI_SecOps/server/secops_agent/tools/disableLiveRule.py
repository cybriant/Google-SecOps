...
def disableLiveRule(
    ...
    """disable Latest Version For A Rule.
    ...
    """
    url = f"https://backstory.googleapis.com/v2/detect/rules/{rule_id}:disableLiveRule"
...
def disable_live_rule_tool(environment: str = "default", rule_id: str = "") -> dict:
    """disables the latest version of a specified Chronicle detection rule.
    ...
    try:
        ...
        result = disableLiveRule(credentials, rule_id)

        return {"status": "success", "result": result, "message": f"Latest version disabled for rule ID: {rule_id}"}
